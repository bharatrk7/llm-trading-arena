"""
DeepSeek LLM Trading Bot for the Arena
"""
...
BOT_API_KEY = os.environ.get('DEEPSEEK_BOT_API_KEY')  # Arena API key for this bot
...
def get_trading_decisions(portfolio: dict) -> list:
    """Ask DeepSeek for trading decisions"""
...
"""
    ...
    try:
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "You are a quantitative portfolio manager. Respond only with valid JSON."},
...
def run():
    print("=" * 50)
    print("🔮 DEEPSEEK TRADING BOT")
    print("=" * 50)
    
    # Validate config
    if not all([ARENA_URL, BOT_API_KEY, DEEPSEEK_API_KEY]):
        print("❌ Missing environment variables!")
        print(f"   ARENA_URL: {'✓' if ARENA_URL else '✗'}")
        print(f"   DEEPSEEK_BOT_API_KEY: {'✓' if BOT_API_KEY else '✗'}")
    ...
    # Get trading decisions from DeepSeek
    print(f"\n🧠 Asking DeepSeek for trading decisions...")
    ...
    print("\n" + "=" * 50)
    print("✅ DeepSeek bot run complete!")
